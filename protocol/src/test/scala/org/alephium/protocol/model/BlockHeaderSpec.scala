...
class BlockHeaderSpec
  ...
  it should "test fromSameTemplate" in new NoIndexModelGenerators {
    ...
    BlockHeader.fromSameTemplate(header0, header0.copy(version = (header0.version + 1).toByte)) is true
    ...
    BlockHeader.fromSameTemplate(header0, header0.copy(timestamp = TimeStamp.unsafe(posLongGen.sample.get))) is true
    BlockHeader.fromSameTemplate(header0, header0.copy(target = Target.unsafe(bytesGen(4).sample.get))) is true
  }
}
