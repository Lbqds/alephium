...
class MemPoolSpec
  ...
  it should "collect transactions based on gas price" in {
    ...
    pool.collectForBlock(index, Int.MaxValue, _ => true) is AVector.from(
  ...
  it should "handle cross-group transactions" in {
    ...
    pool.size is 1
    pool.collectForBlock(ChainIndex(mainGroup, mainGroup), Int.MaxValue, _ => true).isEmpty is true
  ...
  it should "collect sequential txs" in new SequentialTxsFixture {
    {
      ...
      pool.collectForBlock(chainIndex, 1, _ => false) is AVector.empty[TransactionTemplate]
      pool.collectForBlock(chainIndex, 1, isOutputRefExist) is AVector(parentTx)
      pool.collectForBlock(chainIndex, Int.MaxValue, isOutputRefExist) is AVector(
        parentTx,
        childTx
      )
    }

    {
      ...
      val isOutputRefExist = createIsOutputRefExist(pool)
      pool.collectForBlock(chainIndex, 2, isOutputRefExist).toSet is Set(parentTx0, parentTx1)
      pool.collectForBlock(chainIndex, Int.MaxValue, isOutputRefExist).toSet is Set(
        parentTx0,
        parentTx1,
        childTx
      )
...

    {
      ...
      val isOutputRefExist = createIsOutputRefExist(pool)
      pool.collectForBlock(chainIndex, 3, isOutputRefExist).toSet is Set(parentTx0, parentTx1, tx0)
      pool.collectForBlock(chainIndex, Int.MaxValue, isOutputRefExist).toSet is Set(
        parentTx0,
        parentTx1,
        tx0,
...

    {
      ...
      val isOutputRefExist = createIsOutputRefExist(pool)
      pool.collectForBlock(chainIndex, Int.MaxValue, isOutputRefExist) is AVector(parentTx)
      pool.collectForBlock(chainIndex1, Int.MaxValue, isOutputRefExist) is AVector
...

    {
      ...
      val isOutputRefExist = createIsOutputRefExist(pool)
      pool.collectForBlock(chainIndex, Int.MaxValue, isOutputRefExist) is AVector(parentTx0)
      pool.collectForBlock(chainIndex1, Int.MaxValue, isOutputRefExist) is AVector(parentTx1)
    }
