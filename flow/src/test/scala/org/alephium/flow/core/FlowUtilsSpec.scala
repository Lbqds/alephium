...
class FlowUtilsSpec extends AlephiumSpec {
  ...
  it should "collect sequential tx if the input is from parent tx" in new SequentialTxsFixture {
    ...
    blockFlow.collectPooledTxsGhost(chainIndex, groupView, 1) is AVector(tx0)
    blockFlow.collectPooledTxsGhost(chainIndex, groupView, 2) is AVector(tx0, tx1)
  ...
  it should "collect sequential txs based on dependency" in new SequentialTxsFixture {
    ...
    val groupView = blockFlow.getMutableGroupView(chainIndex.from).rightValue
    blockFlow.collectPooledTxsGhost(chainIndex, groupView, 1) is AVector(tx1)
    blockFlow.collectPooledTxsGhost(chainIndex, groupView, 2) is AVector(tx1, tx2)
    blockFlow.collectPooledTxsGhost(chainIndex, groupView, 3) is AVector(tx1, tx2, tx3)
  ...
  it should "collect sequential txs based on gas price" in new SequentialTxsFixture {
    ...
    val groupView = blockFlow.getMutableGroupView(chainIndex.from).rightValue
    blockFlow.collectPooledTxsGhost(chainIndex, groupView, 2) is AVector(tx2, tx3)
    blockFlow.collectPooledTxsGhost(chainIndex, groupView, 4) is AVector(tx2, tx3, tx0, tx1)
  ...
  it should "collect all sequential txs" in new SequentialTxsFixture {
    ...
    val groupView = blockFlow.getMutableGroupView(chainIndex.from).rightValue
    blockFlow.collectPooledTxsGhost(chainIndex, groupView, Int.MaxValue) is txs
