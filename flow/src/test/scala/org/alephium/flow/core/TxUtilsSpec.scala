...
class TxUtilsSpec extends AlephiumSpec {
  ...
  "UnsignedTransaction.buildScriptTx" should "fail because of not enough assets" in new BuildScriptTxFixture {
    {
      ...
      test(inputs, ALPH.oneAlph.subUnsafe(defaultGasFee)).isRight is true
      test(inputs, ALPH.oneAlph.addOneUnsafe()).leftValue is "Not enough balance"
    }

    {
      ...
      test(inputs, ALPH.cent(10), (tokenId, ALPH.oneAlph)).isRight is true
      test(
        inputs,
        ALPH.cent(10),
...

    {
      ...
      test(inputs, ALPH.oneAlph.subOneUnsafe()).isRight is true
      test(inputs, ALPH.oneAlph).leftValue is "Not enough balance for gas fee"
    }
  }
  ...
  "UnsignedTransaction.buildScriptTx" should "fail because of not enough ALPH for change output" in new BuildScriptTxFixture {
    {
      ...
      test(inputs, approvedAlph).isRight is true
      test(inputs, approvedAlph.subUnsafe(dustUtxoAmount)).isRight is true
      test(inputs, approvedAlph.subOneUnsafe()).leftValue is "Not enough ALPH for change output"
    }

    {
      ...
      test(inputs, availableAlph, (tokenId, ALPH.oneAlph)).isRight is true
      test(
        inputs,
  ...
  "UnsignedTransaction.buildScriptTx" should "fail because of inputs not unique" in new BuildScriptTxFixture {
    ...
    test(inputs, ALPH.oneAlph).leftValue is "Inputs not unique"
  }
  ...
  "UnsignedTransaction.buildScriptTx" should "fail because of the number of inputs exceeds MaxTxInputNum" in new BuildScriptTxFixture {
    ...
    test(inputs0, ALPH.oneAlph).isRight is true
    test(
      inputs1,
      ALPH.oneAlph
    ).leftValue is "Too many inputs for the transfer, consider to reduce the amount to send, or use the `sweep-address` endpoint to consolidate the inputs first"
